...
| `Version()` | `version.SemVer` | Returns the semantic version of the SDK (`1.5.1`). |
| `VersionString()` | `string` | Returns the formatted version string (e.g., `"1.5.1"`). |
...
### SemVer Format
Versions are structured as `MAJOR.MINOR.PATCH` (e.g., `1.5.1`):
...
```
*(Note: As of version `1.5.1`, all public APIs are in `StateSupported` status with zero deprecated symbols).*
