...
// Registry represents a thread-safe, central catalog of registered extension descriptors and their lifecycle state.
...
// It returns ErrDuplicateExtension if an extension with the same ID is already registered.
func (r *Registry) Register(desc *Descriptor) error {
	...
	if existing, exists := r.descriptors[id]; exists {
		return fmt.Errorf("%w: extension ID '%s' is already registered (name: %s)", ErrDuplicateExtension, id, existing.Name())
	}
...
// Activate transitions a registered extension from StateRegistered or StateInactive to StateActive.
func (r *Registry) Activate(id string) error {
	...
	if _, exists := r.descriptors[cleanID]; !exists {
		return fmt.Errorf("%w: %s", ErrExtensionNotFound, cleanID)
	}

	if r.states[cleanID] == StateActive {
		return fmt.Errorf("%w: extension '%s' is already active", ErrAlreadyActive, cleanID)
	}
...
// Deactivate transitions an active extension to StateInactive.
func (r *Registry) Deactivate(id string) error {
	...
	if r.states[cleanID] != StateActive {
		return fmt.Errorf("%w: extension '%s' is already inactive", ErrAlreadyInactive, cleanID)
	}
...
// State returns the current lifecycle state of a registered extension.
func (r *Registry) State(id string) (State, error) {
	...
	if !exists {
		return StateRegistered, fmt.Errorf("%w: %s", ErrExtensionNotFound, cleanID)
	}

	return state, nil
}

// IsActive reports whether the extension with the given ID is in StateActive.
...
// Remove unregisters and removes an extension descriptor and its lifecycle state from the Registry.
...
func (r *Registry) Get(id string) (*Descriptor, error) {
	...
	if !exists {
		return nil, fmt.Errorf("%w: %s", ErrExtensionNotFound, cleanID)
	}

	return desc, nil
}

// Has reports whether an extension with the specified ID is registered.
